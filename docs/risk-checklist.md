...
- Public-entry verification risk: a loopback or remote-host-only health check cannot prove the configured user origin reaches this deployment. Production uses a unique per-attempt release id, checks it from the initiating client without cross-origin redirects, and keeps rollback assets until that check passes.
- Secret-file risk: the current, previous, and newly staged remote `.env` files contain bootstrap, MySQL, root, and Redis secrets and must remain mode `0600` before copy and after every edit.
...
- Logout-revocation risk: clearing the browser cookie does not invalidate a copied token. Persist its hash before MySQL deletion, fail closed when the tombstone file is unreadable, and remove tombstones only after the database delete commits.
