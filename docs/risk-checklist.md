...
- Deployment recovery risk: archives and previous directories must be release-specific, while switch/finalize operations share a remote lock. A finalizer that does not carry its release id can delete another in-flight deployment's only rollback asset.
...
- Preference recovery risk: a rejected patch and base snapshot must be persisted before reconciliation starts, later edits must update the durable version, and an older GET must never clear newer recovery state.
...
- Local preference fallback risk: an explicitly unconfigured MySQL backend must disable server preference POSTs; otherwise the documented localStorage mode creates permanent retry traffic.
...
- Tick-grid risk: directional up/down rounding must use the exchange tick itself; applying a later display-precision round can cross the requested direction and create an illegal order price.
- Realtime ordering risk: exchange event time, not local receipt time, owns price/depth ordering. Price and depth streams advance independently so a newer depth snapshot cannot roll back a later bookTicker.
- Auth-scope risk: any ambiguous or failed `/api/auth/me` response must stop boot and preference sync. `auth_enabled` must be a Boolean; only strict false plus the exact numeric id-0 local admin may select local scope, while true requires a positive database user id.
...
- Public-entry verification risk: a loopback or remote-host-only health check cannot prove the configured user origin reaches this deployment. Production uses a unique per-attempt release id, checks it from the initiating client without cross-origin redirects, and keeps its release-specific rollback asset until that check passes.
- Secret-file risk: the current, previous, and newly staged remote `.env` files contain bootstrap, MySQL, root, and Redis secrets and must remain mode `0600`. Generated passwords must never be echoed to SSH or CI output.
- Request-lifecycle risk: the timeout must remain active through `response.json()`, not stop at headers. Market analysis has a 135-second budget; other dashboard APIs use 15 seconds. A 401 must close SSE and stop preference/auth/strategy timers before redirect.
...
- Fallback-retention risk: `BIAN_SIGNAL_REVIEW_LIMIT` is a per-user limit. Retention must not evict other users, and all-user due work must not apply that limit before scanning and sorting every retained scope.
- Logout-revocation risk: clearing the browser cookie does not invalidate a copied token. Logout must run before current-session authentication, persist the token hash before MySQL deletion, surface `revoked=false` to the user, fail readiness when the tombstone store is unusable, and remove tombstones only after the database delete commits.
- Public-health load risk: detailed auth readiness opens MySQL and must be cached. Anonymous health polling must not execute a user-count query for every request.
